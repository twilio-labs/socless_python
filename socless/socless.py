...
__all__ = ['socless_bootstrap', 'socless_log', 'socless_gen_id', 'socless_credentials', 'fetch_from_vault',
           'socless_template_string', 'socless_execute_playbook', 'socless_dispatch_outbound_message',
           'socless_create_events', 'socless_save_state_execution_result', 'socless_fetch_execution_result',
           'socless_save_to_vault', 'socless_post_human_response', 'socless_log_then_raise']
...
def socless_dispatch_outbound_message(receiver, message_id, investigation_id, execution_id, message):
    ...
    try:
        stepfunctions.start_execution(stateMachineArn=STORE_ACTIVITY_TOKEN_ARN,
...
def socless_post_human_response(message_id, response_body):
    ...
    Raises:
        Exception: A blanket exception raised with either of the below message codes
            ...
            await_token_not_found - Await token not available in the Message Responses Table.
                                    It may not have been retrieved yet or retrieval failed
    ...
    try:
        ...
        results_resp = results_table.get_item(Key={"execution_id": execution_id},
    ...
    # README: Below code includes state_name with result so that parameters can be passed
    # to choice state in the same way
    ...
    # TODO: Once Choice states in current playbooks have been updated to the new_style, update this code so
    #  result's are only nested under state_name
...
def socless_create_events(event_data, dedup=True):
    ...
    if not created_at:
        ...
        try:
            ...
            raise Exception("Error: Supplied 'created_at' field is not ISO8601 millisecond-precision string, "
                            "shifted to UTC")
    ...
    for detection in details:
        ...
        if dedup_keys:
            ...
            try:
                scan_results = event_table.scan(FilterExpression=FilterExpression,
...
def socless_bootstrap(event, context, handler, include_event=False):
    ...
    # README: Below code includes state_name with result so that parameters can be passed to choice
