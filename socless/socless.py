...
__all__ = [
    ...
    "socless_execute_playbook",
    "socless_dispatch_outbound_message",
    "socless_create_events",
    "socless_save_state_execution_result",
    "socless_fetch_execution_result",
    "socless_save_to_vault",
    "socless_post_human_response",
...
def socless_save_to_vault(content):
    ...
    vault.put_object(
        Key=file_id, Body=content
    )  # TODO: Should I try catch or let it fail here
    result = {"file_id": file_id, "vault_id": "{}{}".format(VAULT_TOKEN, file_id)}
    return result
...
def socless_execute_playbook(playbook, entry, investigation_id=""):
    ...
    try:
        query_result = playbook_table.get_item(Key={"StateMachine": playbook}).get(
            "Item", False
        )
    except Exception as e:
        socless_log.error(
            "Playbook table query failed", dict(meta, **{"error": f"{e}"})
        )
    ...
    return {
        "status": True,
        "message": {"execution_id": execution_id, "investigation_id": investigation_id},
    }


def socless_dispatch_outbound_message(
    ...
    try:
        ...
        socless_log_then_raise(
            ...
            {"error": f"{e}", "message_id": message_id},
        )
    return True
...
def socless_post_human_response(message_id, response_body):
    ...
    try:
        ...
        results_resp = results_table.get_item(
            Key={"execution_id": execution_id}, ProjectionExpression="results.artifacts"
        )
    except Exception as e:
    ...
    stepfunctions = boto3.client("stepfunctions")
    socless_save_state_execution_result(
        execution_id, receiver, resp_body_with_state_name
    )
    try:
    ...
    for detection in details:
        ...
        if dedup_keys:
            ...
            dedup_expression = " and ".join(
                ["details.{key} = :{key}".format(key=key) for key in dedup_fields]
            )
            if dedup_expression:
...
def socless_save_state_execution_result(execution_id, state_name, result):
    ...
    try:
        ...
        socless_log.error(
            "Failed to save state execution results", dict(meta, **{"error": f"{e}"})
        )
...
def socless_fetch_execution_result(execution_id, state_name):
    ...
    if not item:
        ...
        raise Exception(
            "Error: Unable to get execution_id {} from {}".format(
