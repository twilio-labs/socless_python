...
def dict_to_item(raw, convert_root=True):
    ...
    if isinstance(raw, dict):
        item = {"M": {k: dict_to_item(v) for k, v in raw.items()}}
    elif isinstance(raw, list):
        item = {"L": [dict_to_item(v) for v in raw]}
...
def mock_execution_results_table_entry():
    ...
    client.put_item(
        TableName=results_table_name, Item=dict_to_item(context, convert_root=False)
    )
    return {
        "id": random_id,
        "execution_id": execution_id,
...
def mock_sfn_db_context():
    ...
    sfn_context = {
        ...
        "sfn_context": {
            ...
            "State_Config": {"Name": "Test state name", "Parameters": {}},
        },
    }
    return {
        "id": item_metadata["id"],
