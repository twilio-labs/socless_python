...
from socless.socless import fetch_actual_parameters, fetch_from_vault, socless_save_to_vault, parse_parameters, \
    apply_conversion_from, socless_template_string
import os
import boto3
...
class Test_fetch_actual_parameters(object):
    ...
    def test_with_list_input(self):
        assert fetch_actual_parameters(self.test_data['statics']['list'], self.test_data) \
               == self.test_data['statics']['list']

    def test_with_object_input(self):
        assert fetch_actual_parameters(self.test_data['data']['object'], self.test_data) \
    ...
    def test_nested_reference(self):
        assert fetch_actual_parameters(self.test_data['nested_referer']['object'], self.test_data) \
...
class Test_socless_template_string(object):
    ...
    def test_unsafe_string(self):
        assert socless_template_string("Hello {context.unsafe_string}",
                                       self.context) == "Hello &lt;script&gt;alert('Elliot Alderson')&lt;/script&gt;"

    def test_dictionary_reference(self):
        assert socless_template_string("Hello {context.dict}",
                                       self.context) == """Hello {'safe_string': 'Elliot Alderson', 
                                       'unsafe_string': "&lt;script&gt;alert('Elliot Alderson')&lt;/script&gt;"}"""

    def test_maptostr(self):
        assert socless_template_string("{context.unicodelist|maptostr}", self.context) == "{}".format(
            ['hello', 'world'])
