...
from socless.socless import (
    fetch_actual_parameters,
    fetch_from_vault,
    socless_save_to_vault,
    parse_parameters,
    apply_conversion_from,
...
TEST_DATA = {
    ...
    "referencer": {"param": "$.data.parameter"},
    "statics": {"list": ["1", "2", "3"]},
    "nested_referer": {"object": {"top_level": "$.data.nested_ref"}},
}
...
PARSE_TEST_DATA = {
    ...
    "parameters": {
        "test": {
            ...
            "nested_ref": {"nester": "$.artifacts.event.nested"},
        }
    },
}
...
def test_path_pointing_to_string():
    assert (
        fetch_actual_parameters("$.data.string", TEST_DATA)
...
def test_path_pointing_to_vault_id():
    assert (
        fetch_actual_parameters("$.data.vault_id", TEST_DATA)
...
def test_path_pointing_to_object():
    assert (
        fetch_actual_parameters("$.data.object", TEST_DATA)
...
def test_with_list_input():
    assert (
        fetch_actual_parameters(TEST_DATA["statics"]["list"], TEST_DATA)
        == TEST_DATA["statics"]["list"]
    )


def test_with_object_input():
    assert (
        fetch_actual_parameters(TEST_DATA["data"]["object"], TEST_DATA)
        == TEST_DATA["data"]["object"]
    )


def test_jsonpath_with_json_conversion():
    assert fetch_actual_parameters("$.data.string_json!json", TEST_DATA) == [
        "hello",
        "world",
    ]
...
def test_vault_path_with_json_conversion():
    assert fetch_actual_parameters(
        "vault:socless_vault_tests.json!json", TEST_DATA
    ) == {"hello": "world"}
...
def test_nested_reference():
    assert fetch_actual_parameters(
        TEST_DATA["nested_referer"]["object"], TEST_DATA
...
def test_socless_save_to_vault_saves_cotent_correctly():
    ...
    content = (
        s3.Bucket(os.environ["SOCLESS_VAULT"])
        .Object(result["file_id"])
        .get()["Body"]
        .read()
        .decode("utf-8")
    )
...
def test_socless_fetch_from_vault():
    assert fetch_from_vault("socless_vault_tests.txt") == {
        "content": "this came from the vault"
    }
